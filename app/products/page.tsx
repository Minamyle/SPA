...
export default function ProductsPage() {
  ...
  const brands = searchParams.get('brands') ? searchParams.get('brands')!.split(',').filter(Boolean) : [];
  const categories = searchParams.get('categories') ? searchParams.get('categories')!.split(',').filter(Boolean) : [];
  ...
  }), [currentPage, search, brand, category, brands, categories, priceMin, priceMax, status, sort]);
  ...
  const updateUrlParams = useCallback((updates: Partial<ProductFilters & { page?: number }>) => {
  ...
  const hasClientFilters = !!(filters.brand || filters.priceMin || filters.priceMax || filters.status);
  
  // Use filtered total for pagination when client filters are active
  const effectiveTotal = productsData ? (hasClientFilters && productsData.filteredTotal !== undefined ? productsData.filteredTotal : productsData.total) : 0;
  const totalPages = effectiveTotal > 0 ? Math.ceil(effectiveTotal / filters.limit) : 0;
              ...
              <div className="flex items-center justify-between text-sm text-gray-600 dark:text-gray-400">
                <div className="flex flex-col gap-1">
                  <div className="flex items-center gap-2">
                    <p>
                      ...
                      {Math.min(currentPage * filters.limit, effectiveTotal)} of {effectiveTotal}
                      {hasClientFilters ? ' filtered' : ''} products
                      {hasClientFilters && productsData.total !== effectiveTotal && (
                        <span className="text-gray-500">({productsData.total} total)</span>
                      )}
                    </p>
                    {isFetching && (
                  ...
                  {hasClientFilters && (
                    <div className="flex items-center gap-2 text-xs">
                      ...
                      {filters.brand && <span className="px-2 py-1 bg-blue-100 dark:bg-blue-900 text-blue-700 dark:text-blue-300 rounded">Brand: {filters.brand}</span>}
                      {(filters.priceMin || filters.priceMax) && (
                        <span className="px-2 py-1 bg-blue-100 dark:bg-blue-900 text-blue-700 dark:text-blue-300 rounded">
                          Price: {filters.priceMin ? `$${filters.priceMin}` : '$0'} - {filters.priceMax ? `$${filters.priceMax}` : '∞'}
                        </span>
                      )}
                      {filters.status && <span className="px-2 py-1 bg-blue-100 dark:bg-blue-900 text-blue-700 dark:text-blue-300 rounded">Stock: {filters.status}</span>}
                      <button 
                        onClick={() => updateUrlParams({ brand: '', priceMin: undefined, priceMax: undefined, status: undefined })}
                ...
                <div className="flex items-center gap-2">
                  ...
                  <select
                    id="sort"
                    value={sort}
                    onChange={(e) => updateUrlParams({ sort: e.target.value as ProductFilters['sort'] })}
                    className="rounded border border-gray-300 px-2 py-1 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-800"
                  >
